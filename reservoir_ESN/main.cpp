...
int main(void) {
	...
	for (int r = 0; r < unit_sizes.size(); r++) {
		...
		std::vector<std::string> function_names = { "TDE_ikeda", "TDE_MG",          };// "tanh", "sinc"は時間あれば
			...
			for (int loop = 0; loop < TRIAL_NUM; loop++) {//論文 p12 ばらつき低減
				for (int ite_p = 7; ite_p <= 10; ite_p += 1) {//論文　手順２
					...
					for (int ite_input = 1; ite_input <= 4; ite_input += 1) {//入力ゲイン(τ = 95 pa = 2 ノード100の時は 1～1.3付近で最適なリザバーが出来上がっていた(あと、NARMAタスク, d_bias = 0.4 d_alpha = 0.05, d_sigma = 0.07))
						...
						//const double input_gain = 0.2 + ite_input * 0.2;
				 
						const double input_gain = 0.2 + ite_input * 0.05;
						//const double input_gain = 0.2 + ite_input * 0.04;
