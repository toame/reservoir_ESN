...
int main(void) {
	...
	std::vector < std::vector<std::vector<std::vector<double>>>> w(alpha_step * sigma_step, std::vector<std::vector<std::vector<double>>>(class_num, std::vector<std::vector<double>>(lambda_step))); // 各リザーバーの出力重み
	std::vector<std::vector<std::vector<double>>> nmse(alpha_step * sigma_step, std::vector<std::vector<double>>(class_num, std::vector<double>(lambda_step)));
	for (int r = 0; r < unit_sizes.size(); r++) {
		...
		for (int phase = 0; phase < PHASE_NUM; phase++) {
			if (task_name == "narma") {
				d_bias = 0.4;
				d_alpha = 0.2; alpha_min = 0.2;
				d_sigma = 0.05; sigma_min = 0.6;
				const int tau = param1[r];
				for (int k = 0; k < 60; k++) {
					std::vector<double> noise_signal, narma_signal;
					std::vector<std::vector<double>> teacher_signal_tmp;
					generate_input_signal_random(noise_signal, -1.0, 2.0, small_step + 50, phase * 100 + k);
					generate_narma_task(noise_signal, narma_signal, tau - (k % 2) * 5, small_step + 50);
					generate_classification_task(narma_signal, teacher_signal_tmp, class_num, k % 2);
					for (int t = 50; t < narma_signal.size(); t++) {
						input_signal[phase].push_back(narma_signal[t]);
					}
					for (int i = 0; i < class_num; i++) {
						for (int t = 50; t < teacher_signal_tmp[i].size(); t++) {
							teacher_signal[phase][i].push_back(teacher_signal_tmp[i][t]);
						}
					}
				}
			}
			//for (int i = 0; i < input_signal[phase].size(); i++) {
			//	std::cerr << i << " "<< input_signal[phase][i] << std::endl;
			//}
		}
		...
		for (auto function_name : function_names) {
			...
			for (int loop = 0; loop < TRIAL_NUM; loop++) {
				for (int ite_p = 0; ite_p <= 5; ite_p += 1) {
					const double p = ite_p * 0.2;
					...
					for (int ite_b = 0; ite_b <= 0; ite_b += 1) {
						...
						for (int k = 0; k < alpha_step * sigma_step; k++) {
							...
							for (int j = 0; j <= unit_size; j++) {
								A[k][j] = output_learning[k].A[j][j];
							}
							for (i = 0; i < class_num; i++) {
								output_learning[k].generate_simultaneous_linear_equationsb(output_node[k][TRAIN], teacher_signal[TRAIN][i], wash_out, step, unit_size);
								double opt_lm = 0;
								double opt_lm_nmse = 1e+9;
								for (lm = 0; lm < 10; lm++) {
									for (int j = 0; j <= unit_size; j++) {
										output_learning[k].A[j][j] += pow(10, -15 + lm);
										if (lm == 0) output_learning[k].A[j][j] = A[k][j] + pow(10, -15 + lm);
									...
									nmse[k][i][lm] = calc_nmse(teacher_signal[VAL][i], output_learning[k].w, output_node[k][VAL], unit_size, wash_out, step, false);
								}
							}
						}
						...
						for (int k = 0; k < alpha_step * sigma_step; k++) {
							...
							for (int lm = 0; lm < 10; lm++) {
								if (nmse[k][0][lm] < opt_nmse) {
									const double diff = opt_nmse - nmse[k][0][lm];
									opt_nmse = nmse[k][0][lm];
									...
									//train_nmse = calc_nmse(teacher_signal[TRAIN][i], opt_w, output_node[opt_k][TRAIN], unit_size, wash_out, step, false);
									train_nmse = calc_correct_rate(teacher_signal[TRAIN][0], w[k][0][lm], w[k][1][lm], output_node[opt_k][TRAIN], unit_size, wash_out, step, false, "test");
									//if(diff > 1e-5) std::cerr << opt_nmse << " " << train_nmse << std::endl;
								}
							}
						}
					}
