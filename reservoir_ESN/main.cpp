...
int main(void) {
	...
	const int task_num = 50;   // 時系列のデータ数
	const int class_num = 1;	// クラス数
	const int step = 100;		// 1データの時系列長さ
	const int wash_out = 0;
	std::vector<int> unit_sizes = {100, 100};
	std::vector<std::string> task_names = { "narma", "narma"};
	if (unit_sizes.size() != task_names.size()) return 0;
	std::vector<int> param1 = { 10, 15 };
	...
	std::vector<std::vector<std::vector<std::vector<double>>>> output_node(alpha_step * sigma_step, std::vector<std::vector<std::vector<double>>>(PHASE_NUM, std::vector<std::vector<double>>(step * task_num * class_num + 2, std::vector<double>(MAX_NODE_SIZE + 1, 0))));
	...
	for (int r = 0; r < unit_sizes.size(); r++) {
		...
		std::vector<std::vector<std::vector<std::vector<double>>>> input_signals(PHASE_NUM), teacher_signals(PHASE_NUM);
		...
		for (int phase = 0; phase < PHASE_NUM; phase++) {
			std::vector<std::vector<double>> input_signal, teacher_signal;
			if (task_name == "narma") {
				d_bias = 0.4; 
				d_alpha = 0.005; alpha_min = 0.002;
				d_sigma = 0.07; sigma_min = 0.5;
				const int tau = param1[r];
				//generate_input_signals_random(input_signals[phase], -1.0, 2.0, step, phase + 1);
				//generate_narma_task(input_signals[phase], teacher_signals[phase], tau, step);
				generate_input_signal_random(input_signal, -1.0, 2.0, task_num, step, phase + 1);
				generate_narma_task(input_signal, teacher_signal, tau, step);
				input_signals[phase].push_back(input_signal);
				teacher_signals[phase].push_back(teacher_signal);
			}
			...
			//	const int tau = param1[r];
			//	generate_input_signals_random(input_signals[phase], -1.0, 2.0, step, phase + 1);
			//	generate_narma_task2(input_signals[phase], teacher_signals[phase], tau, step);
			//}
			...
			//	generate_henom_map_task(input_signals[phase], teacher_signals[phase], fstep, step, phase * step);
			//}
			...
			//	const int fstep = param1[r];
			//	generate_laser_task(input_signals[phase], teacher_signals[phase], fstep, step, phase * step);
			//}
			...
			//	generate_input_signals_random(input_signals[phase], -1.0, 2.0, step, phase + 1);
			//	task_for_function_approximation(input_signals[phase], teacher_signals[phase], nu, tau, step, phase);
			//}
		}
		// 設定出力
...
//#pragma omp parallel for num_threads(32)
						...
						for (int k = 0; k < alpha_step * sigma_step; k++) {
							...
							reservoir_layer1.reservoir_update(input_signals[TRAIN], output_node[k][TRAIN], step);
							reservoir_layer1.reservoir_update(input_signals[VAL], output_node[k][VAL], step);
							//is_echo_state_property[k] = reservoir_layer1.is_echo_state_property(input_signals[VAL]);
...
//#pragma omp parallel for  private(lm) num_threads(32)
						...
						for (int k = 0; k < alpha_step * sigma_step; k++) {
							...
							output_learning[k].generate_simultaneous_linear_equationsA(output_node[k][TRAIN], wash_out, class_num, task_num, step, unit_size);
							output_learning[k].generate_simultaneous_linear_equationsb(output_node[k][TRAIN], teacher_signals[TRAIN], wash_out, step, unit_size);
							...
							for (lm = 0; lm < 10; lm++) {
								...
								nmse[k][lm] = calc_nmse(teacher_signals[VAL], output_learning[k].w, output_node[k][VAL], unit_size, wash_out, step, false);
						...
						for (int k = 0; k < alpha_step * sigma_step; k++) {
							...
							for (int lm = 0; lm < 10; lm++) {
								if (nmse[k][lm] < opt_nmse) {
									...
									//train_nmse = calc_nmse(teacher_signals[TRAIN], opt_w, output_node[opt_k][TRAIN], unit_size, wash_out, step, false);
								}
							}
					...
					//opt_reservoir_layer.reservoir_update(input_signals[TEST], output_node_test, step);

					//test_nmse = calc_nmse(teacher_signals[TEST], opt_w, output_node_test, unit_size, wash_out, step, true, output_name);
					...
					// リザーバーのユニット入出力を表示
					//opt_reservoir_layer.reservoir_update_show(input_signals[TEST], output_node_test, step, wash_out, output_name);
					//if (test_nmse > 2.0) {
					//	for (int k = 0; k < alpha_step * sigma_step; k++) {
					//		if (!is_echo_state_property[k]) continue;
					//		for (int lm = 0; lm < 10; lm++) {
