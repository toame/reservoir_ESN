...
void output_learning::generate_simultaneous_linear_equationsA(const std::vector<std::vector<double>>& output_node, const int wash_out, const int step, const int n_size) {
	...
	int count = step - wash_out;//ある程度先のものを使用。それまでは「安定させるため」
	...
	//cblas_dgemm 行列演算
	cblas_dgemm(CblasRowMajor, CblasTrans, CblasNoTrans, n_size + 1, n_size + 1, count, 1.0 / count, sub_output_node.data(), n_size + 1, sub_output_node.data(), n_size + 1, 0.0, B.data(), n_size + 1);//https://qiita.com/nek0log/items/a90c337ea50a11dd71d4  qiita.comが分かりやすい
	for (int n1 = 0; n1 <= n_size; n1++) {//https://www.xlsoft.com/jp/products/intel/perflib/mkl/11.2/mkl_tutorial_c/GUID-36BFBCE9-EB0A-43B0-ADAF-2B65275726EA.htm
		for (int n2 = 0; n2 <= n_size; n2++) {
			A[n1][n2] = B[n1 * (n_size + 1) + n2];//行列を1行に直して格納
		}
	}
}
//連立一次方程式Aw=bのbを生成
