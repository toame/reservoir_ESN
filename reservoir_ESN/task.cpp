...
void generate_input_signal_henon_map2(std::vector<double>& input_signal, const int fstep, const int step, const int wash_out) {
	...
	input_signal[1] = b;
	for (int t = 2; t <= step + fstep; t++) {
		input_signal[t] = 1.0 - alpha * input_signal[t - 1] * input_signal[t - 1] + beta * input_signal[t - 2];// +dist(mt) * É–;
	}
...
double calc_mean_squared_average(const std::vector<double>& teacher_signal, const std::vector<double>& weight,
	...
	std::ofstream outputfile("output_predict_STDE/" + name + ".txt", std::ios::app);//predict ó\ë™
	if (show)
	...
	for (int t = wash_out + 1; t < step; t++) {
		...
		for (int n = 0; n <= unit_size; n++) {
			reservoir_predict_signal += weight[n] * output_node[t + 1][n];//ó·äO
		}
...
double calc_nrmse(const std::vector<double>& teacher_signal, const std::vector<double>& weight,
	...
	double y_max = -1e+9;
	double y_min = 1e+9;
	std::ofstream outputfile("output_predict/" + name + ".txt", std::ios::app);//predict ó\ë™
	if (show)
		outputfile << "t,predict_test,teacher" << std::endl;
	for (int t = wash_out + 1; t < step; t++) {
	...
	//return rmse;//ïœçXó\íËÅ@https://www.sciencedirect.com/science/article/pii/S1463500313001418
