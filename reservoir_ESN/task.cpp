...
void generate_input_signal_henon_map2(std::vector<double>& input_signal, const int fstep, const int step, const int wash_out) {
	...
	for (int t = 2; t <= step + fstep; t++) {
    //for (int t = 2; t <= step ; t++) {
		input_signal[t] = 1 - alpha * input_signal[t - 1] * input_signal[t - 1] + beta * input_signal[t - 2];// +dist(mt) * É–;
	}
...
double calc_mean_squared_average(const std::vector<double>& input_signal, const std::vector<double>& teacher_signal, const std::vector<double>& weight,
	const std::vector<std::vector<double>>& output_node, const int unit_size, const int wash_out, const int step, bool show, std::string name) {//å„Ç≈íºÇ∑
...
double calc_nmse(const std::vector<double>& input_signal, const std::vector<double>& teacher_signal, const std::vector<double>& weight,
	const std::vector<std::vector<double>>& output_node, const int unit_size, const int wash_out, const int step, bool show, std::string name) {
	return (calc_mean_squared_average(input_signal, teacher_signal, weight, output_node, unit_size, wash_out, step, show, name) / t_tt_calc(teacher_signal, wash_out, step));//å„Ç≈íºÇ∑
}