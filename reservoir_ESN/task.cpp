...
#include <string>
#include <sstream>
void generate_input_signal_random(std::vector<std::vector<double>>& input_signals, const int u_min, const int u_delta, const int num, const int step, const int seed) {
	std::mt19937 mt(seed);
...
//  0.3, 0.05, 1.5, 0.1
void generate_narma_task(std::vector<std::vector<double>>& input_signal, std::vector<std::vector<double>>& teacher_signals, int tau, int step) {
	...
	for (int i = 0; i < input_signal.size(); i++) {
		for (int t = 0; t < step; t++) {
			input_signal[i][t] = (input_signal[i][t] + 1) / 4;
		}
		std::vector<double> teacher_signal(step);
		for (int t = 0; t < step; t++) {
			double sum = 0.0;
			if (t - tau >= 0) {
				...
				teacher_signal[t] =
					alpha * teacher_signal[t - 1] + beta * teacher_signal[t - 1] * sum + gamma * input_signal[i][t - tau] * input_signal[i][t] + delta;
...
double calc_mean_squared_average(const std::vector<std::vector<std::vector<double>>>& teacher_signal, const std::vector<double>& weight,
	...
	for (int t = 0; t < step; t++) {
		...
		sum_squared_average += squared(teacher_signal[0][0][t] - reservoir_predict_signal);
		if (show) {
			outputfile << t << "," << reservoir_predict_signal << "," << teacher_signal[0][0]
				[t] << "," << sum_squared_average << std::endl;
		}
	}
	return sum_squared_average / (step - wash_out);
}

double calc_nmse(const std::vector<std::vector<std::vector<double>>>& teacher_signal, const std::vector<double>& weight,
	const std::vector<std::vector<double>>& output_node, const int unit_size, const int wash_out, const int step, bool show, std::string name) {
	...
	return (calc_mean_squared_average(teacher_signal, weight, output_node, unit_size, wash_out, step, show, name));
}